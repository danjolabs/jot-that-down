...
**Deferred.** This was stage 6. It is unscheduled now, not cancelled — the content below is
unchanged from when it was a numbered stage, and nothing depends on it, which is what made moving it
free. Give it a number again when it is next in line.

**Why it moved.** After five stages the thing worth doing next was refining what two surfaces
already do, rather than adding a third. Stage 6 became consolidation; see
[`../stages/stage6.md`](../stages/stage6.md).

**What is still true while it waits.** The seam it depends on — surfaces never touch the filesystem
or SQLite — is built, exercised by two surfaces, and documented in `crates/README.md`. So is
`jot_core::watch`, which lives in core rather than in the TUI *because* this plan needs the same
events. Deferring the surface did not strand either.
...
**Why then.** By that point the domain is settled and exercised by two surfaces, so the desktop app
is a view layer rather than a place where new rules get invented. That was the whole reason it came
last, and it is why waiting costs it nothing.

**Not in this plan.** Mobile, sync, plugins, collaborative anything.
...
- **Logic leaking into the frontend.** The most likely failure of this work, and the one that quietly
