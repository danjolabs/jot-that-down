...
| **M32 `init` mints a constant workspace id** | none | 2 | **Real acceptance gap, and a bad one.** Criterion 1 checks `is_uuid_v7(id)` but never that two `init`s differ. U5 keys the entire registry by workspace id; a constant id collapses every vault a user owns into one entry. Fix: `init` two vaults, assert the ids differ. |
