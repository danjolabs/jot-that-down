...
The user asked for "a basic CLI interface first before stage 2 and 3" — the old numbering, in
which the index was stage 2 and this work was stages 3 and 4 — enough to create a
...
ask was restated: implement stages 2 and 3 in full, **skipping the SQLite layer**, so the core could
...
Skipping stage 4 is only safe if nothing in stages 2–3 *requires* a database. It does not. Threads,
...
deliberately shaped like the tables it stands in for, with a documented one-to-one map from stage 4's
queries to its methods. The **public** `Workspace` API is what `overview.md` specifies either way —
including `sync()` and `rebuild()` with their real signatures — so stage 4 is a substitution behind
the seam rather than a rewrite in front of it. `stage4.md` now carries a "What the snapshot leaves
...
Stage 1b's log recommended going back to the loop for stage 4 "or at minimum dispatch the verifier
...
- The thread algebra is checked against the **worked example printed in `stage2.md`** — both `paths`
...
- All six invariants `stage2.md` lists as properties are asserted, but over a **fixed corpus of eight
...
- `crates/jot-acceptance/` was **not** touched. It remains the verifier's, and there is still no
  independent stage-3 or stage-4 acceptance suite. `crates/jot-cli/tests/cli.rs` covers the stage 3
...
Recommendation for stage 4, restated more strongly than last time: dispatch the verifier separately,
and have it write stage 2 and 3 acceptance suites retroactively before building the index on top of
them.
...
8, which is what git actually does. Written up in `stage3.md`, along with the two consequences that
...
read and possibly repair a second file, which is `open_note`'s job. Recorded in `stage2.md`.
...
| Planned | Built | Why |
| --- | --- | --- |
| `timeline() -> Page<NoteMeta>` | `Page<Row>` | A list view needs reply counts and its parent's state; per-row lookups are the N+1 `stage2.md` names as the stage's trap. |
...
Not covered by the gate, and stated plainly: the `jot new` latency budget (needs stage 4 and a large
