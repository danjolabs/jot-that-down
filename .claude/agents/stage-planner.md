...
- **Three parallel implementers is the ceiling.** Beyond that, integration costs more than the
  parallelism saves.
- **Route by failure mode.** Failure would be _wrong_ (bad invariant, unrepresentable state, a bug
  that surfaces in a year) → opus. Failure would be _incomplete_ (missing flag, thin fixture, caught
